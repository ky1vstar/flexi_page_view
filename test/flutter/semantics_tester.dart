...
class TestSemantics {
  ...
  bool _matches(
    ...
    if (inputType != node.inputType) {
      return fail('expected node id $id to have input type $inputType but found input type ${node.inputType}');
    }
    ...
    if (linkUrl?.toString() != node.linkUrl?.toString()) {
      return fail('expected node id $id to have link url $linkUrl but found link url ${node.linkUrl}');
    }
    ...
    if (identifier != node.identifier) {
      return fail('expected node id $id to have identifier $identifier but found identifier ${node.identifier}');
    }
...
class SemanticsTester {
  ...
  static String _actionsToSemanticsActionExpression(dynamic actions) {
    ...
    if (actions is int) {
      list = SemanticsAction.values.where((SemanticsAction action) => (action.index & actions) != 0);
    } else {
